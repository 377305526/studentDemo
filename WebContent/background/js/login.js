				...
				if (checkAccount(inp) && checkPass(pass)) {
					...
					$.ajax({
						...
						success : function(data) {
							...
							window.location.href = "/DM-2/front/index.jsp";
								...
								if (checkPhone(phone) && checkPass(pcode)) {
									$
											.ajax({
												...
												success : function(data) {
													if (data == 'success') {
														window.location.href = "./register/background/index.jsp";
													} else {
														alert("登录失败！");
													}
			...
			.delegate(
					...
					function() {
						...
						if (checkPhone(phone)) {
							...
							var oTime = $(".form-data .time"), oSend = $(".form-data .send"), num = parseInt(oTime
									.text()), oEm = $(".form-data .time em");
							$(this).hide();
